...
class NemotronSTTUnsupportedError(RuntimeError):
    """Raised when the local runtime cannot transcribe with Nemotron STT."""
...
class FoundryClient:
    """Wraps Foundry Local SDK 1.1.x chat and Nemotron STT clients."""
    ...
    def initialize(self) -> None:
        ...
        logger.info(
            ...
            self._chat_model.alias,
            self._stt_model.alias,
        )

    def _resolve_alias(self, requested: str, fallbacks: tuple[str, ...], kind: str) -> str:
        """Return the requested alias if available, else the first fallback."""
    ...
    def transcribe(self, audio_path: str | Path) -> str:
        ...
        try:
            ...
            msg = str(exc)
            if (
                "nemotron_speech is not a registered multi-modal model type" in msg
